...
model = PeftModel.from_pretrained(base, config['dpo_training']['save_dir'])
...
output_dir = config['evaluation']['dpo_out']
...
for label, questions in categories.items():
    ...
    # save by category
    output_file = os.path.join(output_dir, f"dpo_model_outputs_{label.replace('/', '_')}.json")
